#####################################################
# HelloID-Conn-Prov-Target-Topdesk-Operators-ImportPermission-PermissionGroups
# PowerShell V2
#####################################################
...
try {      
    ...
    foreach ($account in $existingAccounts) {
        $splatGetPermissionGroups = @{
            Uri         = "$($actionContext.Configuration.baseUrl)/tas/api/operators/id/$($account.id)/permissiongroups"
            Headers     = $headers
            Method      = 'GET'
            ContentType = 'application/json; charset=utf-8'
        }
          
        [array]$existingPermissions = Invoke-RestMethod @splatGetPermissionGroups
