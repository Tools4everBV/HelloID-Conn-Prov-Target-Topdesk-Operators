#####################################################
# HelloID-Conn-Prov-Target-Topdesk-Operators-GrantPermission-PermissionGroups
# PowerShell V2
#####################################################
...
try {
    ...
    if (-Not($actionContext.DryRun -eq $true)) {          
        ...
        Write-Information "Granting permission group $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Id)) to $($actionContext.References.Account)"
        $splatParams = @{
        ...
        Write-Information "Successfully granted permission group $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Id)) to $($actionContext.References.Account)"

        $outputContext.AuditLogs.Add([PSCustomObject]@{
                Action  = "GrantPermission"
                Message = "Successfully granted permission group $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Id)) to $($actionContext.References.Account)"
                IsError = $false
            })
    }
    else {
        # Add an auditMessage showing what will happen during enforcement
        Write-Warning "DryRun: Would grant permission group permission $($actionContext.References.Permission.Id) to [$($personContext.Person.DisplayName)]"
    }
}
catch {
    $ex = $PSItem
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        if (-Not [string]::IsNullOrEmpty($ex.ErrorDetails.Message)) {
            $errorMessage = "Could not grant permission group permission: $($ex.ErrorDetails.Message)"
        }
        else {
            $errorMessage = "Could not grant permission group permission Error: $($ex.Exception.Message)"
        }
    }
    else {
        $errorMessage = "Could not grant permission group permission. Error: $($ex.Exception.Message) $($ex.ScriptStackTrace)"
    }
