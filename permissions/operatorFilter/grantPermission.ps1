...
try {
    ...
    if (-Not($actionContext.DryRun -eq $true)) {          
        ...
        Write-Information "Granting operator filter $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Reference)) to $($actionContext.References.Account)"
        $splatParams = @{
        ...
        Write-Information "Successfully granted operator filter $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Reference)) to $($actionContext.References.Account)"

        $outputContext.AuditLogs.Add([PSCustomObject]@{
                Action  = "GrantPermission"
                Message = "Successfully granted operator filter $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Reference)) to $($actionContext.References.Account)"
                IsError = $false
            })
    }
    else {
        # Add an auditMessage showing what will happen during enforcement
        Write-Warning "DryRun: Would grant operator filter permission $($actionContext.References.Permission.Reference) to $($personContext.Person.DisplayName)"
    } 
