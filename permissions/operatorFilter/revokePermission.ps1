#####################################################
# HelloID-Conn-Prov-Target-Topdesk-Operators-RevokePermission-OperatorFilters
# PowerShell V2
#####################################################
...
try {
    ...
    if (-Not($actionContext.DryRun -eq $true)) {          
        ...
        Write-Information "Revoking operator filter $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Reference)) from $($actionContext.References.Account)"
        $splatParams = @{
        ...
        Write-Information "Successfully revoked operator filter $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Reference)) from $($actionContext.References.Account)"

        $outputContext.AuditLogs.Add([PSCustomObject]@{
                Action  = "RevokePermission"
                Message = "Successfully revoked operator filter $($actionContext.PermissionDisplayName) ($($actionContext.References.Permission.Reference)) from $($actionContext.References.Account)"
                IsError = $false
            })
    }
    else {
        # Add an auditMessage showing what will happen during enforcement
        Write-Warning "DryRun: Would revoke operator filter $($actionContext.References.Permission.Reference) from $($personContext.Person.DisplayName)"
    }
}
catch {
    $ex = $PSItem
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        if (-Not [string]::IsNullOrEmpty($ex.ErrorDetails.Message)) {
            $errorMessage = "Could not revoke operator filter permission: $($ex.ErrorDetails.Message)"
        }
        else {
            $errorMessage = "Could not revoke operator filter permission Error: $($ex.Exception.Message)"
        }
    }
    else {
        $errorMessage = "Could not revoke operator filter permission. Error: $($ex.Exception.Message) $($ex.ScriptStackTrace)"
    }
