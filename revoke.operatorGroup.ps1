...
try {
    if ($dryRun -eq $false) {
        ...
        $auditLogs.Add([PSCustomObject]@{
                ...
                Message = "Successfully revoked Permission $($pRef.Name) ($($pRef.id)) from ($($aRef))"
                IsError = $false
            })
    }
}
    ...
    $auditLogs.Add([PSCustomObject]@{
            Action  = "RevokePermission"
            Message = "Failed to revoke permission $($pRef.Name) ($($pRef.id)) from $($aRef.loginName) ($($aRef.id)):  $_"
            IsError = $True
        });
    $success = $false
    Write-Warning $_;
