#####################################################
# HelloID-Conn-Prov-Target-Topdesk-Operators-Update.personData
#
# Version: 2.0.1
#####################################################
...
function Get-TopdeskPersonByCorrelationAttribute {
    ...
    if ([string]::IsNullOrEmpty($responseGet.id)) {
        # no results found
        $errorMessage = "No $($PersonType) found with [$CorrelationAttribute] [$($account.$CorrelationAttribute)]."
        $auditLogs.Add([PSCustomObject]@{
            Message = $errorMessage
            IsError = $true
        })
...
function Get-TopdeskOperatorByCorrelationAttribute {
    ...
    if ([string]::IsNullOrEmpty($responseGet.id)) {
        # no results found
        $errorMessage = "No operator found with [$CorrelationAttribute] [$($account.$CorrelationAttribute)]."
        $auditLogs.Add([PSCustomObject]@{
            Message = $errorMessage
            IsError = $true
        })
